...
import { Star, ThumbsUp, Calendar, User, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function MovieReviews({ reviews, movieId }: MovieReviewsProps) {
  ...
  const allReviews = [
    ...((Array.isArray(userReviews) ? userReviews : []).map((review: any) => ({
      ...
      author_details: {
        avatar_path: review.image || null,
        rating: review.rating
      },
      content: review.content,
      created_at: review.createdAt ? new Date(review.createdAt).toISOString() : new Date().toISOString(),
      updated_at: review.updatedAt ? new Date(review.updatedAt).toISOString() : new Date().toISOString(),
      is_user_review: String(review.user_id) === String(currentUserId)
    })) as any[]),
          ...
          <CardContent className="pt-6">
            <div className="flex flex-col items-center space-y-4">
              ...
              <div className="space-y-2">
                ...
                <p className="text-text-sub text-sm">Be the first to share your thoughts about this movie.</p>
              </div>
            </div>
          </CardContent>
        </Card>
