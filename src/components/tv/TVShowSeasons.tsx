...
const EpisodeCard = memo(({ 
  ...
  <div
    className="group/episode relative w-full bg-gradient-to-br from-[#1b263b] to-[#2e3c51]/40 rounded-xl border border-[#2e3c51]/30 hover:border-[#4fd1c5]/40 transition-colors duration-200 cursor-pointer overflow-hidden"
    onClick={(e) => {
      e.stopPropagation();
  ...
  >
    ...
    <div className="relative w-full aspect-video rounded-t-xl overflow-hidden">
      ...
      <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent flex items-center justify-center opacity-0 group-hover/episode:opacity-100 transition-opacity duration-200">
        <div className="w-12 h-12 bg-gradient-to-r from-[#4fd1c5] to-[#36c7b8] rounded-full flex items-center justify-center shadow-lg">
          <Play className="w-5 h-5 text-[#0d1b2a] ml-0.5" fill="currentColor" />
        </div>
      </div>
    ...
    <div className="p-4 space-y-3 w-full">
      <h5 className="font-semibold text-[#e0e6ed] text-sm line-clamp-2 group-hover/episode:text-white break-words">
        {episode.name}
      </h5>
      ...
      <div className="flex items-center justify-between text-xs text-[#9aafc3] flex-wrap gap-2">
        {episode.air_date && (
          <div className="flex items-center gap-1 min-w-0">
            ...
            <span className="truncate">{format(new Date(episode.air_date), "MMM d")}</span>
          </div>
        )}
        {episode.runtime && (
          <div className="flex items-center gap-1 min-w-0">
...
const SeasonCard = memo(({ 
  ...
  <Card 
    className="group w-full bg-gradient-to-br from-[#1b263b] to-[#2e3c51]/20 border-[#2e3c51] hover:border-[#4fd1c5]/50 transition-colors duration-200 hover:shadow-lg hover:shadow-[#4fd1c5]/10 overflow-hidden will-change-auto"
  >
        ...
        {isLoading || !seasonDetails?.has(season.season_number) ? (
          <div className="p-4 w-full">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 w-full">
              {Array.from({ length: Math.min(season.episode_count, 8) }).map((_, i) => (
          ...
          <div className="p-4 max-h-[600px] overflow-y-auto w-full transform-gpu will-change-scroll scrollbar-thin scrollbar-track-[#1b263b] scrollbar-thumb-[#4fd1c5]/30 hover:scrollbar-thumb-[#4fd1c5]/50" style={{ scrollBehavior: 'smooth' }}>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 w-full">
              {seasonDetails
                .get(season.season_number)
                ?.episodes.map((episode: TMDBEpisode, episodeIndex: number) => (
                  <EpisodeCard
                    key={episode.id}
                    episode={episode}
                    seasonNumber={season.season_number}
                    episodeIndex={0} // Remove staggered animation to reduce lag
...
const TVShowSeasons = ({ tvShow }: TVShowSeasonsProps) => {
  ...
  const handleEpisodeClick = useCallback((seasonNumber: number, episodeNumber: number) => {
        ...
        <div className="flex flex-col sm:flex-row justify-center sm:justify-start items-center gap-4 flex-wrap">
          <div className="flex items-center gap-3 bg-[#2e3c51]/50 rounded-full px-4 py-2 border border-[#4fd1c5]/20 min-w-0 shrink-0">
            ...
            <span className="text-sm font-semibold text-[#9aafc3] whitespace-nowrap">
              <span className="text-[#4fd1c5]">{tvShow.seasons.length}</span> Seasons
            </span>
          </div>
          <div className="flex items-center gap-3 bg-[#2e3c51]/50 rounded-full px-4 py-2 border border-[#4fd1c5]/20 min-w-0 shrink-0">
            <div className="w-2 h-2 bg-gradient-to-r from-[#4fd1c5] to-[#36c7b8] rounded-full"></div>
